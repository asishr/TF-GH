...
variable "name_private_nic_ip" {
   description = "(Required) The Name which should be used for the Private IP Adress Configuration."
  type = string
  default = "ipconfig-trust"
}

variable "name_public_nic_ip" {
  description = "(Required) The Name which should be used for the Public IP Adress Configuration."
  type = string
  default = "ipconfig-untrust"
}

variable "name_mgmt_nic_ip" {
  description = "(Required) The Name which should be used for this Managemnt IP Adreess Configuration."
  type = string 
  default = "ipconfig-mgmt"
}
...
variable "create_natgw" {
  description = "Create or source (when `false`) an existing NatGW. Created or sourced: the NatGW will be assigned to a subnet created by the `vnet` module"
  default     = true
  type        = bool
}

variable "create_pip" {
  description = "Create a Public IP that will be attached to a NatGW"
  default     = true
  type        = bool
}

variable "existing_pip_name" {
  description = "Name of an existing Public IP that will be attached to a NatGW when 'create_pip' is set to false"
  default     = null
  type        = string
...
variable "existing_pip_resource_group_name" {
  description = "Name of the resource group where the existing Public IP is located  when `create_pip` is set to `false`"
  default     = null
  type        = string
}

variable "create_pip_prefix" {
  description = "Create a Public IP Prefix that will be attached to a NatGW"
  default     = false
  type        = bool
}

variable "pip_prefix_length" {
  description = "The length of the Public IP Prefix, can bet between 0 and 31 but this actually supported value depends on the Subscription."
  default     = null
  type        = number
}

variable "existing_pip_prefix_name" {
  description = "Name of an existing Public IP Prefix that will be attached to a NatGW when 'create_pip_prefix' is set to false"
  default     = null
  type        = string
}

variable "existing_pip_prefix_resource_group_name" {
  description = "Name of the resource group where the existing Public IP Prefix is located  when `create_pip_prefix` is set to `false`"
  default     = null
  type        = string
}

variable "zone" {
  description = "Availability Zone to use for the NatGW. If not set, the NatGW will be created without an Availability Zone."
  default     = null
  type        = string
}

variable "idle_timeout" {
  description = "The idle timeout in minutes. The acceptable values for idle timeout are between 4 and 120 minutes. The default value is 4 minutes."
  default     = null
  type        = number
}
